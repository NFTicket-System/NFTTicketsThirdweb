...
import {Badge, Card, Col, Container, Grid, Row, Spacer, Text} from '@nextui-org/react'
...
const CategoryPage = () => {
    ...
    const fetchCategory = async (libelle: string) => {
        let url = `${process.env.NEXT_PUBLIC_API_HOSTNAME}/api/events/single/category/${libelle}`
        await axios
                .get(url)
                .then((response) => {
                    ...
                    <Grid css={{margin: '1%'}}>
                        <Card>
                            <Card.Body css={{position: 'absolute', zIndex: 1, top: 5}}>
                                <Badge enableShadow disableOutline size="xl">
                                    {category ?
                            ...
                            <Card.Image
                                    ...
                                    src={
                                        category ? category!.urlImage : 'https://images.unsplash.com/photo-1507608869274-d3177c8bb4c7?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1470&q=80'
                                    }
                                    objectFit="cover"
                                    width="100%"
                                    height={250}
