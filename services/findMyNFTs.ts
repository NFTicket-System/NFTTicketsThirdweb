...
export async function findMyNFTs({ connectedAddress }: { connectedAddress: string }) {
	...
	await axios.get(process.env.NEXT_PUBLIC_API_HOSTNAME + '/api/tickets/all/collection/addresses').then((response) => {
		response.data.map((item: { addressContract: string }) => collections.push(item.addressContract))
	})
